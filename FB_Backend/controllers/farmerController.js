...
const Seller = require('../models/farmerModel');
...
const signup = async (req, res) => {
    try {
        ...
        return res.status(200).send({message: "Farmer account created"});
    } catch (error) {
        if(error.code===11000){
            if (error.keyPattern.email || error.keyPattern.phoneNo) {
                return res.status(400).send({message: 'Farmer with this email or phone number already exists'});
            }
